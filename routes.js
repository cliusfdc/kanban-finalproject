...
module.exports = function(app) {
  ...
  app.get("/api/tasks", function(req, res) {
    const query =
      "SELECT Id, AssignedName__c, Title__c, TaskDescription__c, Status__c, DueDate__c, Color__c FROM Kanban__c";
  ...
  // ** Need to see if this is written correctly**
  app.delete("/api/tasks/:id", function(req, res) {
    console.log("routes.js app.delete() called");
    const record = nforce.createSObject("Kanban__c");
    record.set("Id", req.params.id);
    ...
    conn.delete({ sobject: record }, (err, data) => {
      if (!err) {
        ...
        console.log("conn.delete failed");
        res.json(err);
      }
    });
  });
